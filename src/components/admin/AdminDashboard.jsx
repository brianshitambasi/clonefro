...
import { fetchServices, fetchProjects, fetchJobs, fetchContacts, fetchUsers } from '../../api/api';
import { useAuth } from '../../context/AuthContext'; // ✅ Fixed path - ../../context/AuthContext

const AdminDashboard = () => {
  const { user } = useAuth(); // Now this will work
  const [stats, setStats] = useState({ services: 0, projects: 0, jobs: 0, contacts: 0, users: 0 });
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>Dashboard</h2>
        <span className="badge bg-primary fs-6">Welcome, {user?.name || 'Admin'}</span>
      </div>
          ...
          <div key={card.title} className="col-md-4 col-lg-3">
            <div className={`card border-0 shadow-sm h-100 text-${card.color}`}>
              <div className="card-body">
                ...
                <p className="card-text display-6 fw-bold">{card.count}</p>
                <a href={card.link} className="stretched-link text-decoration-none"></a>
              </div>
            </div>
          </div>
        ))}
